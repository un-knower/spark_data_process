...
//      .filter(_._2._6.toString.nonEmpty)
    ...
    val join_rdd = bill16_rdd.join(bill17_rdd).map(x=>(x._1,(x._2._1.toString().concat(",").replace("(", "").replace(")", "")
    ...
    val out_rdd = join_rdd.leftOuterJoin(user_rdd).map( x=>(
      x._1,
