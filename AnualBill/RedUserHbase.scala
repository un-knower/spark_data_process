...
object RedUserHbase {
  ...
  private val bill_input = hdfs_path+"/user/yimr/reduser/DWA_USER_YEAR_END_RED_*"
  ...
  def main (args: Array[String]) {
    ...
    //输出格式：(用户手机号，())
    val bill16_rdd=sc.textFile(bill_input).filter(_.split("\\|", -1).length > 15).filter(_.split("\\|", -1)(12).equals("2016"))
      .map(x => (x.split("\\|", -1)(2),//电话号
      (x.split("\\|", -1)(13),
        x.split("\\|", -1)(15),
        x.split("\\|", -1)(5),
        x.split("\\|", -1)(6),
        x.split("\\|", -1)(3),
        x.split("\\|", -1)(4)))).filter( _._1.toString.nonEmpty)
    //      .filter(_._2._2.toString.nonEmpty)
    //      .filter(_._2._4.toString.nonEmpty)
    //      .filter(_._2._6.toString.nonE对mpty)

    //输出格式：(用户手机号，())
    val bill17_rdd = sc.textFile(bill_input).filter(_.split("\\|", -1).length > 15).filter(_.split("\\|", -1)(12).equals("2017"))
      .map(x => (x.split("\\|", -1)(2),//电话号
      (x.split("\\|", -1)(13),
        x.split("\\|", -1)(15),
        x.split("\\|", -1)(5),
        ...
        x.split("\\|", -1)(3),
        x.split("\\|", -1)(4)))).filter( _._1.toString.nonEmpty)
      .filter(x=>x._2._1.toString.equals("") || x._2._3.toString.equals("") || x._2._5.toString.equals(""))
    //      .filter(_._2._2.toString.nonEmpty)
    //      .filter(_._2._4.toString.nonEmpty)
    //      .filter(_._2._6.toString.nonEmpty)

    val bill17_not = sc.textFile(bill_input).filter(_.split("\\|", -1).length > 15).filter(_.split("\\|", -1)(12).equals("2017"))
    println("bill16:"+bill16_rdd.count())
    println("bill17:"+bill17_rdd.count())
    //join后格式：(用户手机号，（（16），（17））)
    ...
    println("joinrdd:"+join_rdd.count())
